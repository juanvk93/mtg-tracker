...
- **Sistema de diseño (CSS)**: todo el estilo vive en `static/css/main.css` con tokens en `:root` (y sus overrides oscuros). Acento dorado `--brand` (+ `--brand-bg`), podio `--oro`/`--plata`/`--bronce`, `--shadow`/`--shadow-hover`, y `--font-display` (Cinzel, para titulares/números; body en Inter). Convenciones: KPIs destacados con `.stat-card.kpi-hero`; en el ranking, fila del líder `tr.fila-lider` y posiciones `.rank-1/2/3`; win rate como píldora `.win-rate.alta/media/baja`. Los KPIs de líder por victorias y por win rate se fusionan en una tarjeta MVP cuando es el mismo jugador (lógica en `inicio.html`).
