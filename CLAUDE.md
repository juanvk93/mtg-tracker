...
- **La versión aparece en DOS sitios**: `internal/version/version.go` (`Version` + changelog) y **hardcodeada** en el texto del enlace de `templates/base.html` (p. ej. `v1.6.0`). Al publicar una versión hay que actualizar ambos y añadir una entrada al `Changelog()`.
