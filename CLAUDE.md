...
- **Jugadores inactivos** (`jugadores.activo`): se desactivan sin borrarlos. **Siguen en el ranking y en el histórico**, pero **se ocultan de Estadísticas** (fichas por jugador, gráficas de evolución, matriz de dominancia, premios, selects de head-to-head y también como verdugo/víctima de otros — ver el filtrado por `.Activo` en el handler `Estadisticas` y en `ObtenerVerdugoYVictima`). Los cálculos históricos (p. ej. posición media) sí cuentan a los inactivos que participaron. Los formularios de resultados usan `ObtenerJugadoresActivos` **más** los que ya participaron en esa sesión (para poder editar sesiones antiguas).
